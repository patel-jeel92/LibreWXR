...
- **Precipitation nowcasting (experimental)** — 60-minute short-range forecast by extrapolating recent radar forward using optical flow, with configurable blend mode: pure radar extrapolation (default, closest to Rain Viewer), smooth radar-to-model blending, or pure NWP forecast. The model side is taken from the active NWP chain — HRRR over CONUS, ICON-EU/DINI over Europe, IFS elsewhere. Beyond 60 minutes, always uses pure model. Quality varies by weather pattern — works best for steady, organized precipitation; less reliable for fast-developing convection
...
| `LIBREWXR_NOWCAST_BLEND_MODE` | `radar` | `radar`, `blended`, or `model`. Beyond 60 min always uses pure model |
| **Satellite + alerts** | | |
